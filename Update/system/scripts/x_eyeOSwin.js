...
//  var id = xGetElementById (ev.currentTarget.id.substring (0, ev.currentTarget.id.length-4));
  ...
  if (document.getElementById && !document.all) event = ev;
  currentid = (document.all ? event.srcElement.id : document.getElementById ? ev.currentTarget.id : null);
  var id = xGetElementById (currentid.substring (0, currentid.length-4));
...
function restoreWin(pID){
 var myObj = document.getElementById(pID);
 if((myObj == null)) {
  window.location.href="?a="+pID;
 }
 if(myObj != null) {
...
function minimizeApp (w) {
  while (w && (!w.className || !w.className.match(/(^|\s+)eyeapp(\s+|$)/)))
    w = w.parentNode;
  var myObj = document.getElementById(w.id);
