...
//	  echo "<pre>appdir : $appdir\n"; print_r ($appfiles); echo '</pre>';
  ...
  if (!empty ($_REQUEST['move']) && !empty ($_REQUEST['app'])) {
     ...
     elseif (($_REQUEST['move'] == "down") && ($actualpos < (count($appsarray) - 1 ))) {
         $nextpos = $actualpos + 1;
...
//-------------------LIST APPS-------------------------
  if (empty ($_SESSION['apps'][$eyeapp]['argv'])) {
     ...
     <a href='?a=$eyeapp(installer)'>
      <img class='imgbar' border='0' alt='"._L('Install Apps')."' title='"._L('Install Apps')."' src='".findGraphic('','new.png')."' \> "._L('Install Apps')."
     </a>");
      addActionBar("
     <a href='?a=$eyeapp(manager)'>
      <img class='imgbar' border='0' alt='"._L('Manage Apps')."' title='"._L('Manage Apps')."' src='".findGraphic('','open.png')."' \> "._L('Manage Apps')."
     </a>");
  ...
  foreach (explode (',', APPDIRS) as $nadir) {
    if ($directory = @opendir ($nadir = filename ($nadir))) {
      while ($pgm = readdir ($directory))
	      if ((false !== ($aInfo = makeApp ("$nadir$pgm", '', false))) && is_file($nadir.$pgm."/".APP_ICON)) {
...
//-------------------MANAGE APPS-------------------------
   elseif (@$appinfo['argv'][0] == 'manager') {
         addActionBar("
     <a href='?a=$eyeapp'>
      <img class='imgbar' border='0' alt='"._L('Go Back')."' title='"._L('Go Back')."' src='".findGraphic('','back.png')."' \>
     </a>");
    if (!empty ($errmsg)) 
	          echo "
...
if (agree) return true; else return false ; }
    ...
    foreach (explode (',', APPDIRS) as $nadir) {
      if ($directory = @opendir ($nadir = filename ($nadir))) {
        while ($pgm = readdir ($directory))
	        if ((false !== ($aInfo = makeApp ("$nadir$pgm", '', false))) && 
	...
	echo "</td><td>$pgm</td>
	  <td align='center'>";
      if ($pgm != basename(APPMANAGER)) echo"
...
if (in_array($nadir.$pgm, $listm = explode (',', @$_SESSION['usrinfo']['apps']))) {
	if ($pgm != basename(APPMANAGER)) echo " checked />";
        if (($nadir.$pgm != $listm[0]) && ($nadir.$pgm != $listm[(count($listm)-1)])) echo " <a href='?a=$eyeapp(manager)&app=$nadir$pgm&move=up'><img border='0' alt='"._L("Move to the left")."' title='"._L("Move to the left")."' src='".findGraphic('','up.png')."' /></a> <a href='?a=$eyeapp(manager)&app=$nadir$pgm&move=down'><img border='0' alt='"._L("Move to the right")."' title='"._L("Move to the right")."' src='".findGraphic('','do.png')."' /></a>";
	elseif ($nadir.$pgm == $listm[0]) echo " <a href='?a=$eyeapp(manager)&app=$nadir$pgm&move=down'><img border='0' alt='"._L("Move to the right")."' title='"._L("Move to the right")."' src='".findGraphic('','do.png')."' /></a>";
        else echo " <a href='?a=$eyeapp(manager)&app=$nadir$pgm&move=up'><img border='0' alt='"._L("Move to the left")."' title='"._L("Move to the left")."' src='".findGraphic('','up.png')."' /></a>";
}
else echo "/>";
	echo "
	  </td>
    ...
    <td> 
      <img src='".findGraphic('','restore2.png')."' alt='"._L('Restore %0',$pgm)."' title='"._L('Restore %0',$pgm)."' style='cursor:pointer;' onclick='restoreApp(\"$pgm\")' border='0' />
     </td>";
	    ...
	    <select name='skin[$nadir$pgm]' >";
	       ...
	       <option".((@basename($aInfo['skin']) == $s) ? ' selected' : '').">$s</option>";
	          ...
	          if ($nadir == USRDIR . USR."/apps/") 
		          echo "<td><a onclick=\"return WRemoveApp()\" href='desktop.php?a=$eyeapp(manager)&app2erase=$pgm'><img border=\"0\" src=\"".findGraphic('','delete.png')."\"></a></td>";

	          if (($nadir == SYSAPPS) && (USR == ROOTUSR)) 
		          echo "<td><a onclick=\"return WRemoveApp()\" href='desktop.php?a=$eyeapp(manager)&app2erase=$pgm&systemwideapp'><img border=\"0\" src=\"".findGraphic('','delete.png')."\"></a></td>";
    ...
    <form action='?a=$eyeapp' enctype='multipart/form-data' method='post'>
	 ...
	 echo "
      ...
      ";
        ...
        echo "
      <input name='Submit' style='border: 0; background-color: transparent; color: #929292;' TYPE='image' SRC='".findGraphic('','upload.png')."'>
    </form>
    </div>";
      }
